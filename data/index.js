...
export function exportData(data, file) {
  ...
  const result = await fetch(
    `https://api.notion.com/v1/databases/${url}/query`,
    {
      ...
      headers: {
        ...
        Authorization: 'Bearer ' + process.env.VITE_NOTION_API_SECRET,
...
async function getNotionFortunes() {
  const response = await fetchContent(process.env.VITE_NOTION_API_FORTUNES);
...
async function getNotionSigns() {
  const response = await fetchContent(process.env.VITE_NOTION_API_SIGNS);
  const { results } = response;
  return results
    .map((item) => {
      const props = item.properties;
...
async function getDos() {
  const response = await fetchContent(process.env.VITE_NOTION_API_DOS);
  const { results } = response;
  return results
    .map((item) => {
      const props = item.properties;
...
async function getDonts() {
  const response = await fetchContent(process.env.VITE_NOTION_API_DONTS);
  const { results } = response;
  return results
    .map((item) => {
      const props = item.properties;
...
async function getHouses() {
  const response = await fetchContent(process.env.VITE_NOTION_API_HOUSES);
  const { results } = response;
  return results
    .map((item) => {
      const props = item.properties;
...
async function getTarot() {
  const response = await fetchContent(process.env.VITE_NOTION_API_TAROT);
  const { results } = response;
  return results
    .map((item) => {
      const props = item.properties;
...
export async function load() {
  ...
  exportData(sentences, 'sentences');
  exportData(signs, 'signs');
  exportData(dos, 'dos');
  exportData(donts, 'donts');
  exportData(houses, 'houses');
  exportData(tarots, 'tarots');
}
